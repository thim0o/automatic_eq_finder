...
class RealTimePlotter:
    ...
    def __init__(self, app):
        ...
        self.freq_plot = self.plot_widget.addPlot(title="Frequency Response", row=0, col=0, colspan=2)
        ...
        self.freq_plot.addLegend(labelTextSize="10pt", brush=pg.mkBrush(45, 45, 45, 150))

        self.error_plot = self.plot_widget.addPlot(title="Optimization Error", row=1, col=0)
        self.error_plot.setLabels(left="Error (dB RMS)", bottom="Iteration")
        self.error_plot.showGrid(x=True, y=True, alpha=0.3)

        self.error_bar_plot = self.plot_widget.addPlot(title="Summary RMS Error", row=1, col=1)
        ...
        self.original_curve = self.freq_plot.plot(pen=pg.mkPen('#00ffff', width=1.5), name="Original")  # Cyan
        self.best_corrected_curve = self.freq_plot.plot(pen=pg.mkPen('#ffff00', width=1.5),
                                                        name="Best Corrected")  # Yellow
        self.latest_corrected_curve = self.freq_plot.plot(pen=pg.mkPen('#ff00ff', width=1.5),
                                                          name="Latest Corrected")  # Magenta
        self.eq_profile_curve = self.freq_plot.plot(
            pen=pg.mkPen('w', width=2, style=QtCore.Qt.PenStyle.DashLine), name="EQ Profile")
        self.target_curve = self.freq_plot.plot(
            pen=pg.mkPen('#00ff00', width=2.5, style=QtCore.Qt.PenStyle.SolidLine), name="Target Curve")  # Bright Green

        # Use a brighter green for better visibility on dark background
        self.error_curve = self.error_plot.plot(pen=pg.mkPen('#00ff00', width=1.5), symbol='o', symbolBrush='#00ff00')

        # Bar graph colors are already bright, so they are fine
        self.original_error_bar = pg.BarGraphItem(x=[0], height=[0], width=0.6, brush='#00ffff')
        self.best_error_bar = pg.BarGraphItem(x=[1], height=[0], width=0.6, brush='#ffff00')
        self.latest_error_bar = pg.BarGraphItem(x=[2], height=[0], width=0.6, brush='#ff00ff')
        ...
        checkboxes_defs = {
            "Original": "chk_original", "Best Corrected": "chk_best", "Latest Corrected": "chk_latest",
            "EQ Profile": "chk_eq_profile", "Target Curve": "chk_target", "Filter Visuals": "chk_filters"
        }
    ...
    def _update_filter_visuals(self, freqs, target_curve_data, filter_list):
        ...
        for fc, gain, q in filter_list:
            ...
            pen_color = pg.mkPen('#ff0000' if gain < 0 else '#ffA500', width=1.5)  # Red for cut, Orange for boost

            gain_line = pg.PlotDataItem(x=[fc, fc], y=[target_level_at_fc, target_level_at_fc + gain], pen=pen_color)
            bandwidth = fc / q
            q_line = pg.PlotDataItem(x=[fc - bandwidth / 2, fc + bandwidth / 2],
                                     y=[target_level_at_fc + gain, target_level_at_fc + gain], pen=pen_color)
    ...
    def update_freq_response(self, freqs, original, best_corrected, latest_corrected, eq_curve, target_curve_data,
                             filter_list):
